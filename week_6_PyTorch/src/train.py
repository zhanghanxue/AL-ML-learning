...
class Trainer:
    ...
    def print_class_accuracy(self, class_correct, class_total):
        ...
        for classname in self.classes:
            ...
            print(
                f"  {classname:15s}: {accuracy:5.1f}% ({correct_count:4d}/{total_count:4d})"
            )
    ...
    def train(self, epochs=3, resume=True):
        ...
        for epoch in range(self.start_epoch, self.start_epoch + epochs):
            ...
            # Print epoch results
            print(
                f"Epoch {epoch+1}/{self.start_epoch + epochs}: "
...
def main():
    ...
    if args.dataset == "cifar10":
        ...
        train_loader, val_loader, classes = get_oxford_pets_dataloaders(
            batch_size=args.bs
        )
        model = OxfordPetsCNN(
            num_classes=len(classes)
        )  # Now outputs 37 classes instead of 10
