...
class LLMService:
    ...
    def __init__(self):
        self.api_url = f"https://api-inference.huggingface.co/models/{settings.HF_MODEL}"
        self.headers = {
    ...
    def generate(self, prompt: str, max_tokens: Optional[int] = None) -> Dict[str, Any]:
        if self.total_cost > settings.FREE_TIER_LIMIT_USD * 0.9:
            logger.warning(f"⚠️  Warning: At 90% of free tier (${self.total_cost:.4f}/${settings.FREE_TIER_LIMIT_USD})")
        ...
        try:
            ...
            if isinstance(result, list):
                ...
                generated_text = str
                logger.warning(f"Unexpected response format from LLM API: {type(result)}")
            ...
            logger.info(f"LLM request #{self.request_count} completed in {response_time_ms}ms, cost: ${cost:.6f}, total cost: ${self.total_cost:.4f}")
    ...
    def get_cost_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "remaining_percentage": (remaining_free_tier / settings.FREE_TIER_LIMIT_USD) * 100,
            "requests": self.request_count,
