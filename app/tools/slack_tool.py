...
class SlackTool:
    ...
    def __init__(self):
        ...
        if not self.enabled:
            logger.warning("Slack webhook not configured. Alerts will be logged but not sent.")
    
    def determine_priority(self, intent: str, confidence: float) -> str:

        # Critical: Immediate attention needed
        if intent in ["card_lost_or_stolen", "fraudulent_transaction"] and confidence > 0.8:
            return "critical"
        
        # High: Urgent but not emergency
        elif intent in ["account_hacked", "urgent_help"] and confidence > 0.7:
            return "high"
        
        # Medium: Should be reviewed soon
        elif intent in settings.URGENT_INTENTS and confidence > 0.6:
            return "medium"
    ...
    def send_alert(self, 
        ...
        # Mock Mode Check
        if not self.enabled:
            logger.info(f"[MOCK SLACK] Would send {priority} alert for intent '{intent}'")
            return {
                "success": True,
...
def send_slack_alert(ticket_text: str, intent: str, confidence: float) -> Dict[str, Any]:
